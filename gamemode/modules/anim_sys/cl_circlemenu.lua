...
local r, x, y = weight_source(400), ScreenWidth / 2, ScreenHeight / 2
local ply = LocalPlayer()
GlobalOptions = {
	[1] = {
		name = "Выбрать стойку",
...
CDRag = CurTime()
local background = circles.New(CIRCLE_OUTLINED, r, x, y, weight_source(200))
background:SetMaterial(true)
background:SetColor(Color(0,0,0,250))


local background2 = circles.New(CIRCLE_FILLED, weight_source(190), x, y, weight_source(100))
...
wedge:SetColor(Color(143, 37, 156, 255) )
...
function NewWheel(opt)
	...
	Wedge2 = circles.New(CIRCLE_OUTLINED, r - 195, x, y, weight_source(10))
	Wedge2:SetColor(Color(143, 37, 156, 255) )
	...
	dbt_emote.wheel.Paint = function( self, w, h )
		...
		local new_a = Lerp(FrameTime() * 10, background:GetRadius(), weight_source(400))

		background:SetRadius(new_a)
		...
		for i = 0, #options - 1 do
			...
			if selected == i then
				...
				if option.data and CurrentEmoteImg then
					draw.SimpleText(
						option.name, "Comfortaa X40", ScreenWidth / 2, ScreenHeight / 2 - hight_source(20),
						color_white,
						TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER
					)

					draw.SimpleText(
						CurrentEmote, "Comfortaa X40", ScreenWidth / 2, ScreenHeight / 2 + hight_source(20),
					...
					draw.SimpleText(
						option.name, "Comfortaa X40", ScreenWidth / 2, ScreenHeight / 2,
						color_white,
						TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER
					)
...
hook.Add( "PlayerButtonDown", "dbt.Emote.Wheel", function( ply, button )
	if ( IsFirstTimePredicted() and 16 == button and not IsValid(dbt_emote.wheel) and not IsClassTrial() and not IsValid(FUCKFRMAE)) then
        ...
        fp_enb = false
		 ...
		 if ply:GetModel() == "models/player/dewobedil/danganronpa/toko_fukawa/default_p.mdl" or ply:GetModel() == "models/player/dewobedil/danganronpa/toko_fukawa/genocide_p.mdl" then
			 GlobalOptions[7] = {
		 		name = "Сменить модель",
		 		mat = materialtoko,
...
function ChangeBodygroupsModel()
	disablebuild = true
	...
	for _, bg in pairs(bodygroups) do
		if bg.num - 1 == 0 then iscluchenia = iscluchenia + 1 continue end
		modelPanel:GetEntity():SetBodygroup(bg.id, ply:GetBodygroup(bg.id))
	    local label = vgui.Create("DLabel", panel)
		surface.SetFont("Comfortaa X30")
		local x, y = surface.GetTextSize(bg.name)
	    label:SetText(bg.name)
		label:SetFont("Comfortaa X30")
		label:SetWidth(x, y + 10)
	    label:SetPos(scrw*0.26, scrh*0.05 * (bg.id - iscluchenia) + scrh*0.03)

	    local slider = vgui.Create("DNumSlider", panel)
	    slider:SetPos(scrw*0.26 + 90, scrh*0.05 * (bg.id - iscluchenia) + scrh*0.03)
	    slider:SetSize(200, 20)
	    slider:SetMin(0)
	    slider:SetMax(bg.num - 1)
	    slider:SetDecimals(0)
	    slider:SetValue(ply:GetBodygroup(bg.id))

	    slider.OnValueChanged = function(self, value)
	        modelPanel:GetEntity():SetBodygroup(bg.id, math.Round(value))
			netstream.Start("ChangeBodyGroup", LocalPlayer(), bg.id, math.Round(value))
	    end
	end

	surface.SetFont("Comfortaa X30")
	local x, y = surface.GetTextSize("Закрыть")

	local button = vgui.Create("DButton", panel)
	button:SetPos(scrw - x - 15, y - scrh*0.03)
	button:SetSize(x, y + 20)
	button:SetText('')

	button.Paint = function(self, w, h) end

	local dlabel = vgui.Create("DLabel", panel)
	dlabel:SetFont("Comfortaa X30")
	dlabel:SetPos(scrw - x - 15, y - scrh*0.03)
	dlabel:SetSize(x, y + 20)
	dlabel:SetText("Закрыть")
...
local rtWidth, rtHeight = 512, 512
local renderTarget = GetRenderTarget("PlayerModelRT", rtWidth, rtHeight, false)
local renderMaterial = CreateMaterial("PlayerMod1elMaterisdasls10", "UnlitGeneric", {
    ["$basetexture"] = renderTarget:GetName(),
   	["$translucent"] = 1,
})
local gradientMaterial = Material("vgui/gradient-u")

hook.Add("HUDPaint", "RenderPlayerModel", function()
	if !ShowEmotions then return end
    render.PushRenderTarget(renderTarget)

    render.Clear(0, 0, 0, 0, true, true)

    local z = model:GetBonePosition( model:LookupBone( "ValveBiped.Bip01_Head1" ) ).z
    cam.Start3D(Vector(15, -4, z + 2), Angle(0, 160, 0), 60, 0, 0, rtWidth, rtHeight)
	...
	dbtPaint.StartStencil()
    	...
    	surface.DrawTexturedRect(50, ScrH() / 2 - 256, rtWidth, rtHeight)
    --char_ico_1
