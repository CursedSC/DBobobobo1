    ...
    for i, bodyPart in ipairs(BODY_PARTS) do
        bodyPartsTable[i + 1] = {
            ...
            func = function()
                ...
                netstream.Start("dbt/medication/started", target)
                
                net.Start("dbt.StartMedicationProcess")
                ...
                dbt.ShowTimerTarget(5, "Применение медикамента...", target, function()
...
netstream.Hook("dbt/medication/started", function(target)
    local ply = LocalPlayer()
    ply.MedicationTarget = target
end)
