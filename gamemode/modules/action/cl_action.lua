...
function MedicationProgress:Start(duration, callback, medData)
    self.active = true
...
local function OpenMedicationProgress(duration, callback, medData)
    MedicationProgress:Start(duration, callback, medData)
end
    ...
    for i, bodyPart in ipairs(BODY_PARTS) do
        bodyPartsTable[i + 1] = {
            ...
            func = function()
                ...
                OpenMedicationProgress(5, function(success)
                    if success then
                        ...
                        chat.AddText(Color(234, 30, 33), "[Лечение] ", Color(255, 255, 255), "Процедура отменена!")
                    end
