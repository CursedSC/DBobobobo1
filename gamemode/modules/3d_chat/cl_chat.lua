...
function BuildNewChat()
    ...
    ChatTextFrame:SetPos( 0, ScrH() - ChatTextFrame:GetTall())
...
SetDragLimit(ChatTextFrame)
    ...
    TextEntry:SetFont("Comfortaa X24")
    ...
    TextEntry.OnFocusChanged = function(self, b)
        netstream.Start("Player.Chating", b)
    end
...
function dbt:AddPlayerSay(ply, text, action, IsSpy, IsLooc, CustomDraw, UseSteamName, IsPM)
    ...
    local x, y = surface.DrawMulticolorText(weight_source(5), weight_source(15), "Comfortaa X24", i.text, 700)
    TextPanel:SetSize(ScrW()*0.375-10, y + weight_source(15))
    if IsValid(ply) then
        TextPanel.ply = ply
        local tempcolor = dbt.chr[i.ply:Pers()].color
    ...
    local rpname = i.ply and dbt.chr[i.ply:Pers()].name
    TextPanel.Paint = function(self, w, h)
        ...
        if i.IsSpy then
            ...
            draw.SimpleText( "Неизвестный", "Comfortaa X24", weight_source(5), 0, self.P_color )
            draw.SimpleText( i.action, "Comfortaa X24", weight_source(5) + ww, 0, ColWhite )
            local x, y =  surface.DrawMulticolorText(weight_source(5), weight_source(20), "Comfortaa X24", i.text, 700, APLHA_PERCENT)
        elseif i.IsLooc then
            local ww, hh = surface.GetTextSize("[LOOC] "..steamname)
            draw.SimpleText( "[LOOC] "..i.ply:Name(), "Comfortaa X24", weight_source(5), 0, self.P_color )
            draw.SimpleText( i.action, "Comfortaa X24", weight_source(5) + ww, 0, ColWhite )
            local x, y =  surface.DrawMulticolorText(weight_source(5), weight_source(20), "Comfortaa X24", i.text, 700, APLHA_PERCENT)
        elseif i.CustomDraw then
            ...
            return
        elseif not ply then
            local x, y =  surface.DrawMulticolorText(weight_source(5), weight_source(5), "Comfortaa X24", i.text, 700, APLHA_PERCENT)
        else
            if i.UseSteamName and !IsPM then
                ww, hh = surface.GetTextSize(steamname)
                draw.SimpleText( steamname, "Comfortaa X24", weight_source(5), 0, self.P_color )
			elseif i.UseSteamName and IsPM then
				ww, hh = surface.GetTextSize(steamname)
                draw.SimpleText( steamname, "Comfortaa X24", weight_source(5), 0, self.P_color )
            else
                ww, hh = surface.GetTextSize(rpname)
                draw.SimpleText( rpname, "Comfortaa X24", weight_source(5), 0, self.P_color )
            end
