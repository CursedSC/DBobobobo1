...
function SWEP:PrimaryAttack()
	...
	if owner:IsValid() then
		if SERVER then
			if dbt.hasWoundOnpos(owner, "Перелом", "Левая рука") and dbt.hasWoundOnpos(owner, "Перелом", "Правая рука") then return end
		end
	end
...
function SWEP:SecondaryAttack()
	...
	elseif self:HandsStatusIs("hands") then
		...
		if SERVER then
			...
			local ent = tr.Entity
			if (ent:GetNWString("Owner") == self.Owner:GetNWString("KeyOwner") and isDoor(ent) and self.cooldown and ent:GetNWString("Owner") != "" or (IsMono(self.Owner:Pers()) and self.cooldown and isDoor(ent))) and self.Owner:GetPos():Distance(ent:GetPos()) < 100 then
				self.cooldown = false
				timer.Simple(1, function() self.cooldown = true end)
				net.Start("rp.OpenAnim")
				net.WriteEntity(self.Owner)
				net.Broadcast()
				if ent.Loccked then
					self.Owner:EmitSound("door_open.mp3")
					ent:Fire("unlock")
					ent.Loccked = false
				else
					self.Owner:EmitSound("door_close.mp3")
					ent:Fire("Lock")
					ent.Loccked = true
				end
			end
...
function SWEP:DealDamage()
	...
	if SERVER and IsValid(tr.Entity) and (tr.Entity:IsNPC() or tr.Entity:IsPlayer()) and tr.Entity:Health() > 20 then
		...
		local attack = dbt.chr[self.Owner:Pers()].fistsDamage
		dmginfo:SetDamage(math.random(attack.min, attack.max))
